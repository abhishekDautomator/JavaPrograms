...
public class CollectionsExample<T> {
    ...
    public static void main(String[] args) {
        ...
        //without stream, it's best to use hashmap
        Map<Character, Integer> freqMap = new HashMap<>();
        for (char c : charArr) {
            freqMap.put(c, freqMap.getOrDefault(c, 0) + 1);
        }
        System.out.println("count of each char in " + Arrays.toString(charArr) + " using hashmap is: " + freqMap);
        ...
        //2nd highest
        int secondHighest = listNum.stream()
                .distinct()
                .sorted(Comparator.reverseOrder())
                .limit(2)
                .skip(1)
        ...
        System.out.println("2nd highest in " + listNum + " is " + secondHighest);

        //How do you sort a stream by multiple fields (e.g., age then name)?
        System.out.println("Sorted person list by age first and then name: ");
        System.out.println(people.stream()
                .sorted(Comparator.comparingInt(CollectorsExample.Person::age))
                .sorted(Comparator.comparing(CollectorsExample.Person::name))
                .toList());

        //How do you count the frequency of elements in a list using streams?
        System.out.println("Frequency of each element in "+listNum+" is: ");
        System.out.println(listNum.stream()
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting())));

        //How do you join a list of strings into a comma-separated string using streams?
        String commaSeparatedString = strings.stream()
                .collect(Collectors.joining(","));

        System.out.println("List of string "+strings+" after getting joined by comma: "+commaSeparatedString);
        System.out.println("Second way of joining list of strings: "+String.join(",", strings));

        //converting a List<String> into a String[] using stream
        String[] strArr = strings.toArray(String[]::new);
        System.out.println("converted a list "+strings+" into an array "+Arrays.toString(strArr));

        //How do you filter and transform a stream in one pass
        List<Integer> squareOfEvenNums = listNum.stream()
                .filter(a->a%2==0)
                .map(a->a*a)
                .toList();
        System.out.println("List of square of even numbers in the list "+listNum+" is: "+squareOfEvenNums);
